...
error_bmmse_cn_imp = zeros(size(SNRs));
...
for SNR = SNRs
    ...
    params.rho = mtk_util_db_to_linear(SNR);
    params = mtk_ce_prepare('blmmse-cn', params);
    for channel_index=1:channel_iterations
        params.H = H(:,:,channel_index);
        params.N = N(:,:,1);
        H_hat = params.H; % Perfect CSI
        H_hat_imp = mtk_ce_estimate("blmmse-cn", params); % Imperfect CSI

        params.x_real = mtk_util_vec_real(params.x);

        params.H_hat = H_hat;
        G_bmmse = mtk_det_detector('bmmse-cn', params);
        params.H_hat = H_hat_imp;
        G_bmmse_imp = mtk_det_detector('bmmse-cn', params);
        for noise_index=1:noise_iterations
            params.n = repmat(n(:,:,noise_index), 1, params.symbol_iterations);
            ...
            x_tilde_real = G_bmmse * y_real;
            x_tilde = mtk_util_vec_complex(x_tilde_real);
            x_hat = mtk_util_demodulate('qpsk', x_tilde);
            current_error_bmmse = current_error_bmmse + sum(x_hat ~= params.x, 'all');

            x_tilde_real = G_bmmse_imp * y_real;
            x_tilde = mtk_util_vec_complex(x_tilde_real);
            x_hat = mtk_util_demodulate('qpsk', x_tilde);
            current_error_bmmse_imp = current_error_bmmse_imp + sum(x_hat ~= params.x, 'all');
    ...
    error_bmmse_cn(snr_index) = current_error_bmmse / (params.K * channel_iterations * symbol_iterations * noise_iterations);
    error_bmmse_cn_imp(snr_index) = current_error_bmmse_imp / (params.K * channel_iterations * symbol_iterations * noise_iterations);
    snr_index = snr_index + 1;
end
toc;
